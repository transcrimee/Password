   ...
   if os.path.exists(profile_json):
    with open(profile_json, "r") as f:
     ...
     while password == "": #Simple check to make sure username is not empty because it just be weird 
      ...
      file_name = input("Enter your file name for your password: ")
     with open(f"password/proflie/{user_proflie}.json", "w") as f: # Creates the username.json and ready to writes the username into it 
     ...
     data = password.encode(master_password)  # Define 'data' here

     key = Fernet.generate_key()
     with open("password/key/lock.key","wb") as file:
      file.write(key)
